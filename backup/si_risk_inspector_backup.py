...
def require_api_key() -> None:
    ...
    if not os.getenv("OPENAI_API_TYPE"):
        st.error("OPENAI_API_TYPE 환경 변수가 설정되어 있지 않습니다. (예: 'azure')")
        st.stop()
    if not os.getenv("OPENAI_API_VERSION"):
        st.error("OPENAI_API_VERSION 환경 변수가 설정되어 있지 않습니다. (예: '2023-05-15')")
        st.stop()
...
def analyze_risks(df: pd.DataFrame, model: str = "gpt-4.1-mini") -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
...
model_choice = st.sidebar.selectbox("사용 모델", "gpt-4.1-mini")

#st.sidebar.write(f"현재 사용 모델(배포 이름): {model_choice}")

if st.sidebar.button("▶ 분석 실행"):
    st.session_state["run"] = True

# ───────────── 데이터 적재 ─────────────
raw_df = load_csvs(uploaded_files)
